...
def comp_vel_rel(VelMax):
    ...
    while (grados_act >= 0 and grados_act <= 360):
        
        Velx= VelMax*cos(grados_act)
        Vely= VelMax*sen(grados_act)
        break

    return Velx,Vely, grados_act


def ir_relativo(dir,VelMax,t):

    Vx, Vy, grados_act = comp_vel_rel(VelMax)

    if (dir=="adel"):
        send_ned_velocity(Vx,Vy,0,t)

    elif (dir=="atras"):
        send_ned_velocity(-Vx,-Vy,0,t)

    elif (dir=="izq"):
        if ((grados_act <= 5 and grados_act >=0) or (grados_act>=355 and grados_act <=360)) or (grados_act<= 100 and grados_act >=85) or (grados_act <= 185 and grados_act>=175) or (grados_act <= 275 and grados_act >=265):
            send_ned_velocity(Vy,-Vx,0,t)

        elif (grados_act > 5 and grados_act <85) or (grados_act > 185 and grados_act <265):
            send_ned_velocity(Vx,-Vy,0,t)

        elif (grados_act > 95 and grados_act <175) or (grados_act > 275 and grados_act< 355):
            send_ned_velocity(-Vx,Vy,0,t)
